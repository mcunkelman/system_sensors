...
def _venv_python_path(venv_path: Path) -> Path | None:
    """Return the Python binary inside a venv, or None if not found.
 
    Checks both 'python3' and 'python' since venvs on different distros
    use different names.
    """
...
def choose_venv_path() -> Path:
    ...
    venv_healthy = venv_exists and _venv_python_path(venv_path) is not None
 
    if venv_exists and not venv_healthy:
        print(f"  Incomplete venv found at {venv_path} — removing.")
...
def create_venv(venv_path: Path) -> Path:
    ...
    if venv_python is None:
        _err(f"Venv created but no Python binary found in {venv_path / 'bin'}")
        _info("Expected 'python3' or 'python' — check your Python installation.")
        sys.exit(1)
...
def install_package(venv_python: Path) -> None:
    ...
    # Upgrade pip first — the bundled pip in a fresh venv may be too old
    # to handle hatchling editable installs correctly.
    print("  Upgrading pip ...")
    result = subprocess.run(
    ...
    if result.returncode != 0:
        _err("pip upgrade failed:")
        _info(result.stderr.strip() or result.stdout.strip())
        sys.exit(1)
 
    print("  Installing system_sensors into venv ...")
    result = subprocess.run(
